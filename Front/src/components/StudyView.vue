<template>
    ...
    <el-scrollbar style="width: 1050px">
        <el-space direction="vertical">
            <div v-for="(Article, index) in res_from_server" @click="handleDetail(res_from_server[res_from_server.length - 1 - index][5])">
                <!-- 这里是每个帖子的抬头，也就是蓝色部分 -->
                <el-card id="card-info" style="height: 60px; background-color:#409EFF">
                    <el-row :span="24" style="font-size:20px; color:white">
                        <el-col :span="8" > 
                            <!-- 0: User_id, 1: Title, 2: Text, 3: Time, 4: Position, 5: Post_id -->
                ...
                <!-- 这里是每个帖子的内容，也就是白色部分 -->
                <el-card class="box-card" style="width: 1000px; height: 280px; display: block; word-wrap: break-word;">
                    <template #header>
                        <div class="card-header">
                            <span style="font-size: 25px"> {{res_from_server[res_from_server.length - 1 - index][1]}}</span>
                        </div>
                    </template>
                    <el-text truncated line-clamp="7"  style="font-size: 20px; display: block; word-wrap: break-word;"> {{ res_from_server[res_from_server.length - 1 - index][2] }} </el-text>
                </el-card>
            </div>
        </el-space>
        <el-divider />
...
function getContent() {
    axios.get("http://localhost:5000/study")
    .then(Response => {
        res_from_server.value = Response.data;
        for(let i = 0; i < res_from_server.value.length; i++) {
            axios.post('http://localhost:5000/get_name_by_id',
            {'User_id': res_from_server.value[i][0] })
                .then(response => {
...
function publishPost() {
    ...
    router.push({ name: 'PublishPost', query: { from: "study" } })
}

function handleDetail(Post_id) {
    router.push({ name: 'PostDetail', query: { Post_id: Post_id } });
}
